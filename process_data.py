...
class ProcessData:
    ...
    def split_data(self, df: pd.DataFrame, target: str, test_size: float, 
        ...
        train_valid, test = train_test_split(df, test_size=test_size, stratify=df[target], random_state=random_state)
        train, valid = train_test_split(train_valid, test_size=val_size, stratify=train_valid[target], random_state=random_state)
    ...
    def save_data(self, out_dir):
        for d in ('X_train', 'X_valid', 'X_test', 'y_train', 'y_valid', 'y_test'):
            self.__getattribute__(d).to_csv(os.path.join(out_dir, f'{d}.csv'), index=False)
